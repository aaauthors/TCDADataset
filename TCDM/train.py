...
def trainer(cfg, args, epoch, model, model_ema, train_loader, valid_loader, criterion, optimizer, lr_scheduler,
    ...
    for e in range(epoch):
        ...
        if cfg.METRIC.TYPE == 'pedestrian':
            ...
            if args.local_rank == 0:
                print(f'mae: {:.4f}, rmse: {:.4f} '.format(train_result.mae,train_result.rmse))

                print(f'mae: {:.4f}, rmse: {:.4f} '.format(valid_result.mae,valid_result.rmse))
...
def argument_parser():
    ...
    parser.add_argument('--local_rank', help='', default=0,
                        type=int)
